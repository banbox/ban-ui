 ...
 * @param drawArgs
 */
export function drawCloudInd (drawArgs: IndicatorDrawParams) {
 ...
 * 绘制缠论的线条
 */
export function drawChanlun({ctx, indicator, visibleRange, xAxis, yAxis}: IndicatorDrawParams): boolean {
  // 指标计算结果
  ...
  // 循环遍历每个线段
  result.forEach((val) => {
    const [start_id, start_price, stop_id, stop_price] = val
 ...
 * @param yAxis
 */
export function drawFigures({ctx, kLineDataList, indicator, visibleRange, defaultStyles, xAxis, yAxis}: IndicatorDrawParams){

...
const ruler: OverlayTemplate = {
  ...
  createPointFigures: ({ coordinates, overlay, precision, barSpace }) => {
    ...
    if (coordinates.length > 1) {
      ...
      const priceChg = valueDif.toFixed(precision.price)
      // @ts-ignore
      ...
      const text = `${priceChg} (${pctChg}%)\n${barNum}${t("num_bar")}, ${getIntervalText(dist_sec)}`
      let textY = pt2.y + 10
      let boxBaseLine = 'top'
      if(pt1.y > pt2.y){
        textY = pt2.y - 10
